package com.burgess.excel.api.parse;
 ...
 * @package com.burgess.excel.api.parse
 * @file ExcelParseServiceImpl.java
 * @author burgess.zhang
 * @time 21:51:09/2018-08-28
 * @desc 导入数据实现
 */
public class ExcelParseServiceImpl implements ExcelParseService {
	...
	public Map<String, Object>[] parse(InputStream inputStream, List<ExcelField> configs) throws ExcelIoInputException,
		...
		logger.info("ExExcelServiceImpl.parse(inputStream={},configs={})", inputStream, configs);
		Map[] dataMap = null;
		if (inputStream == null) {
			logger.error(
		...
		int dataSize = rowNum + 1 - config.getDataIndex();
	...
	private Workbook readWorkbookByInputStream(InputStream inputStream) throws ExcelIoInputException {
		logger.info("ExExcelServiceImpl.readWorkbookByInputStream(inputStream={})", inputStream);
		...
		logger.info("the version of excel file can't analyse,reason:" + sb.toString());
	...
	private Map<Object, Object> readRowHandler(Row row, List<ExcelField> configs) throws ExcelDataTypeHandlerException,
			ExcelCellException, ExcelNotFoundHandlerException, ExcelValidateException {
		logger.info("ExExcelServiceImpl.readRowHandler(row={},configs={})", row, configs);
		...
		for (int i = 0; i < colNum; i++) {
			...
			logger.info(String.format("reading data  the column['%s'] of excel .", i));
			config = mapConf.get(i);
			if (config == null) {
				logger.info(String.format(
						"don't read data  the column['%s'] of excel,because it is not config the column  .", i));
				continue;
			}
			Object value = null;
	...
	private Map<Integer, ExcelField> convertToColMap(List<ExcelField> configs) {
		logger.info("ExExcelServiceImpl.convertToColMap(ColumnConfig={})", configs);
	...
	private Object readCellValue(Cell cell, ExcelField config, int row) throws ExcelCellException,
			ExcelDataTypeHandlerException, ExcelNotFoundHandlerException, ExcelValidateException {
		logger.info("ExExcelServiceImpl.readCellValue(cell={},ColumnConfig={})", cell, config);
		Object value = null;
		Object cellValue = null;
		if (cell != null) {
			logger.info("three is reading excel cell[" + cell.getRowIndex() + "," + cell.getColumnIndex() + "] value ");
			FormulaEvaluator formulaEval = cell.getRow().getSheet().getWorkbook().getCreationHelper()
			...
			case Cell.CELL_TYPE_STRING:
				try {
					...
					throw new ExcelCellException(cell.getRowIndex(), cell.getColumnIndex(), e.getMessage(), e);
				}
				break;
			...
			case Cell.CELL_TYPE_FORMULA:
				...
				try {
					Double doubleValue = formulaEval.evaluate(cell).getNumberValue();
					cellValue = doubleValue;
				} catch (Exception e) {
					throw new ExcelCellException(cell.getRowIndex(), cell.getColumnIndex(), e.getMessage(), e);
				}
		...
		if (validateHandler != null) {
			...
			if (!validate) {
				String msg = String.format("the value[%s] of cell[%d,%d] can't pass the validate handler [%s].  ",
						value, row, config.getColumnIndex(), validateHandler.getClass().getName());
				logger.error(msg);
				throw new ExcelValidateException(validateHandler, value, row, config.getColumnIndex(), msg, null);
			}
		}
		...
		} else if (getDataHandlerService() != null && StringUtils.isNotBlank(config.getDataHandlerName())) {
			logger.info(String.format("DataHandlerService(%s),argtype=%s", String.valueOf(value),
		...
		if (dataHandler != null) {
			...
			logger.info(String.format("%s  k=%s,v=%s", config.getFieldName(), String.valueOf(translateValue), value));
		...
		} else if (getDataTypeService() != null && StringUtils.isNotBlank(config.getDataTypeHandlerName())) {
			logger.info(
					String.format("DataTypeService(%s),argtype=%s", String.valueOf(value), value.getClass().getName()));
			hander = getDataTypeService().find(config.getDataTypeHandlerName());
		}
		if (hander != null) {
			Class<?> paramType = getMethodParamTypes(hander, "converter");
			if (value.getClass() == paramType || paramType.getName().equals("java.lang.Object")) {
				...
				logger.error("ExExcelServiceImpl.readCellValue(cell={},ColumnConfig={}),%s", cell, config, message);
				throw new ExcelDataTypeHandlerException(hander.getClass().getName(), value, row,
						config.getColumnIndex(), message, null);
			}
		}
		return value;
	}
