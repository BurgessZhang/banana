 ...
 * @package com.burgess.excel.handler.stylehandler
 * @file StyleHandlerFactory.java
 * @author burgess.zhang
 * @time 22:24:45/2018-08-28
 * @desc
 */
public class StyleHandlerFactory {
	...
	private StyleHandlerService getStyleHandlerService() {
		...
		ServiceLoader<StyleHandlerService> styleHandlerServiceLoader = ServiceLoader.load(StyleHandlerService.class);
	...
	public CellStyle handler(Cell cell, String style, CellStyle cellStyle)
			throws ExcelStyleException, ExcelStyleHandlerException {
		logger.info("StyleServiceImpl.handler(cell={},style={},cellStyle={})", cell, style, cellStyle);
		if (cellStyle == null) {
			cellStyle = cell.getSheet().getWorkbook().createCellStyle();
		}
		logger.info(
				String.format("setting style[%s] into cell[%d,%d] ", style, cell.getRowIndex(), cell.getColumnIndex()));
		if (StringUtils.isNotBlank(style)) {
			...
			for (String styleKeyValueStr : stylesKeyAndValue) {// 分离所有样式数据
				logger.info(String.format("parsing style[%s] into cell[%d,%d] ", styleKeyValueStr, cell.getRowIndex(),
						cell.getColumnIndex()));
				styleKeyValue = styleKeyValueStr.split(split);// 分离样式的key和value
				if (styleKeyValue == null || styleKeyValue.length != 2) {
					logger.error(String.format("cell[%d,%d]'s style[%s] is illegal,style is the style's [key:value]",
							cell.getRowIndex(), cell.getColumnIndex(), styleKeyValueStr));
					throw new ExcelStyleException(cell.getRowIndex(), cell.getColumnIndex(), styleKeyValueStr,
							String.format("cell[%d,%d]'s style[%s] is illegal,style must be the style's [key:value]",
									cell.getRowIndex(), cell.getColumnIndex(), styleKeyValueStr));
				}
			...
			for (String styleKey : keySet) {// 分别处理每个样式
				...
				logger.info(String.format("setting style[key=%s,value=%s] into cell[%d,%d] ", styleKey, styleValue,
						cell.getRowIndex(), cell.getColumnIndex()));
				logger.info(String.format("setting user-defined style[key=%s,value=%s] into cell[%d,%d] ", styleKey,
						styleValue, cell.getRowIndex(), cell.getColumnIndex()));
				try {
					...
					String msg = String.format("call the name[%s] cell[%d,%d]'s style[key=%s,value=%s] handler error, ",
							styleKey, cell.getRowIndex(), cell.getColumnIndex(), styleKey, styleValue,
							e.getDetailMsg());
					logger.error(msg);
					ExcelStyleHandlerException ex = null;
					if (handler != null) {
						ex = new ExcelStyleHandlerException(handler.getClass().getName(),
								String.format("%s:%s", styleKey, styleValue), msg, e);
					} else {
