...
public class Border implements StyleHandler {
	...
	public CellStyle handler(Cell cell, String style, CellStyle cellStyle) {
		...
		if (styles.length == 1 && StringUtils.isNotBlank(styles[0])) {
			styleValue = styles[0];
		} else if (styles.length == 2 && StringUtils.isNotBlank(styles[1])) {
			location = styles[0];
			styleValue = styles[1];
		}
		...
		logger.info("StyleServiceImpl.border(cell={},location={},styleValue={},cellStyle={})", cell, location,
				styleValue, cellStyle);
		logger.info(String.format(" running border(cell[%d,%d],location[%s],styleValue['%s'])", cell.getRowIndex(),
				cell.getColumnIndex(), location, styleValue));
		if (cellStyle == null) {
